# Prompt for the INPROFIC agent — headless commerce payments

Implement the INPROFIC side of payments for the existing headless commerce integration used by the No Left Overs Nigeria restaurant website. Preserve INPROFIC as the source of truth for order totals, payment state, receivables/finance, and fulfilment. Do not move payment authority into the storefront.

## Existing storefront contract

The NLO website already creates commerce orders through the existing authenticated headless endpoint and expects the order's INPROFIC-generated `public_id` and authoritative `total`. It now supports four payment methods:
...
The website intentionally never sends an amount when initiating payment. INPROFIC must derive the payable amount from the order and its own pricing/finance state.

Implement these authenticated headless endpoints under the same storefront/business slug and `X-INPROFIC-Key` authentication used by the current commerce API:
...
- Derive amount/currency from INPROFIC. Never trust an amount from the storefront even if one is supplied.
...
Return the normalized current payment record, including at least method, status, amount, currency, INPROFIC payment reference, gateway reference when applicable, bank account/instructions when appropriate, `amount_paid`, `balance`, and verification timestamps if available.
...
Make transfer references appropriately unique/deduplicated where possible and audit every claim. If you support proof uploads elsewhere in INPROFIC, link them to the payment attempt, but do not require the headless website to determine validity.

## Paystack

Implement Paystack as a proper server-side gateway integration:

- Store Paystack secret keys only in INPROFIC business/integration settings; never expose them through the headless API.
- Initialize the transaction server-side with the INPROFIC authoritative amount converted to kobo, NGN currency, customer email when available, and INPROFIC payment/order references in metadata.
- Return only the gateway authorization URL/reference needed by the storefront.
- Add a Paystack webhook endpoint in INPROFIC and verify the Paystack signature (`x-paystack-signature`) against the raw request body using the business/integration secret.
...
- Keep API/secret/contract credentials only in INPROFIC configuration.
- Initialize transactions server-side from the INPROFIC amount and return the checkout/payment URL/reference.
...
Bank transfer has no gateway confirmation in this flow, so implement explicit INPROFIC-controlled verification:

- Customer-submitted transfer reference -> `awaiting_verification`, never `paid`.
- Provide an internal INPROFIC UI/action for authorized finance/business staff to review the claim against bank evidence/statement/actual credit.
...
- If confirmed amount is below amount due, use INPROFIC's existing partial-payment/receivable semantics rather than pretending it is fully paid.
...
If INPROFIC already has bank-transaction reconciliation functionality, integrate with it rather than creating a parallel accounting system.

## Cash verification

Cash must also be manually verified in INPROFIC:
...
- Confirmation must capture receiver user, timestamp, amount actually received, location/outlet if INPROFIC models one, and optional note.
...
Use or adapt INPROFIC's existing payment/receivable models instead of building duplicate accounting. At minimum distinguish:
...
- `failed`
- `cancelled`
- `refunded` where existing INPROFIC finance supports it
...
- create exactly one appropriate finance/receipt/ledger effect using existing INPROFIC mechanisms;
...
- Headless endpoints must be tenant-scoped and authenticated with the existing INPROFIC headless credential rules.
...
7. only authorized INPROFIC staff can verify bank transfer/cash;
